...
abstract class AbstractMatch {
    ...
    protected function transformBase64DataUrls() {
        foreach ($this->attributes as $key => $value) {
            $isDataUrl = \DevOwl\RealCookieBanner\Vendor\DevOwl\FastHtmlTag\Utils::isBase64DataUrl($value);
            if ($isDataUrl) {
    ...
    protected function renderAttributes() {
        ...
        return ' ' . \DevOwl\RealCookieBanner\Vendor\DevOwl\FastHtmlTag\Utils::htmlAttributes($attributes);
    }
    /**
    ...
    public function setInvisibleAttribute($key, $value) {
        ...
        $this->setAttribute(
            \sprintf('%s%s', self::HTML_ATTRIBUTE_INVISIBLE_PREFIX, $key),
            $value === null ? null : \base64_encode(\json_encode(['val' => $value]))
        );
    }
    /**
