...
class HeadlessContentBlocker extends \DevOwl\RealCookieBanner\Vendor\DevOwl\FastHtmlTag\FastHtmlTag {
    ...
    protected function processMatch($matcher, $match) {
        ...
        if ($match->hasChanged()) {
            foreach ($match->getAttributes() as $key => $val) {
                $originalAttributeKey = \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\AttributesHelper::revertTransformAttribute(
                    $key
                );
    ...
    protected function runBlockedMatchCallback($result, $matcher, $match) {
        foreach ($this->blockedMatchCallbacks as $callback) {
            ...
            \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\finder\match\MatchPluginCallbacks::getFromMatch(
                $match
            )->runBlockedMatchCallback($result, $matcher);
        }
    }
    /**
     ...
     * @return array[]
     */
    public function getBlockableRulesStartingWith($prefix, $onlyExpressions = \false) {
        $result = [];
        foreach ($this->getBlockables() as $blockable) {
