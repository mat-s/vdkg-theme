 ...
 * ```
 */
class ForceVisual extends \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\AbstractPlugin {
    ...
    public function fn($fn, $match, $value) {
        \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\finder\match\MatchPluginCallbacks::getFromMatch(
            ...
            if ($result->isBlocked()) {
                $match->setAttribute(
                    \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Constants::HTML_ATTRIBUTE_VISUAL_FORCE,
                    \true
                );
            }
        });
