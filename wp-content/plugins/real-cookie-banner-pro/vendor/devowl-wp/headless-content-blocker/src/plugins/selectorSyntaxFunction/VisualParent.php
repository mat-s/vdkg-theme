 ...
 * ```
 */
class VisualParent extends \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\AbstractPlugin {
    ...
    public function fn($fn, $match, $value) {
        ...
        if (!empty($visualParent)) {
            \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\finder\match\MatchPluginCallbacks::getFromMatch(
                $match
            )->addBlockedMatchCallback(function ($result) use ($match, $visualParent) {
                if (
                    $result->isBlocked() &&
                    !$match->hasAttribute(
                    ...
                    $match->setAttribute(
                        \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Constants::HTML_ATTRIBUTE_VISUAL_PARENT_HIDE,
                        \true
                    );
                    ...
                    if (
                        ...
                        !\is_numeric($visualParent)
                    ) {
                        $match->setBeforeTag(
                            $match->getBeforeTag() .
                                \sprintf(
                                    '<style>%s:has([%s="%s"][%s]){visibility:hidden;}</style>',
                                    $visualParent,
                                    \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Constants::HTML_ATTRIBUTE_VISUAL_PARENT,
                                    $visualParent,
