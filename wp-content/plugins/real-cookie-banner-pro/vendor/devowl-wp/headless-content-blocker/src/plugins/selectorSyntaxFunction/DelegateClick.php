 ...
 * ```
 */
class DelegateClick extends \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\AbstractPlugin {
    ...
    public function fn($fn, $match, $value) {
        ...
        \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\finder\match\MatchPluginCallbacks::getFromMatch(
            $match
        )->addBlockedMatchCallback(function ($result) use ($match, $selector, $hide) {
            if ($result->isBlocked()) {
                $match->setAttribute(
                    \DevOwl\RealCookieBanner\Vendor\DevOwl\HeadlessContentBlocker\Constants::HTML_ATTRIBUTE_DELEGATE_CLICK,
                    \json_encode(['selector' => $selector, 'hide' => $hide])
                );
            }
        });
        return \true;
