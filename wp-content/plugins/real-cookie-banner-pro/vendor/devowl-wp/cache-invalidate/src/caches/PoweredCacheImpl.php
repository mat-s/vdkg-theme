 ...
 * @codeCoverageIgnore
 */
class PoweredCacheImpl extends \DevOwl\RealCookieBanner\Vendor\DevOwl\CacheInvalidate\AbstractCache {
    ...
    public function invalidate() {
        return \PoweredCache\Utils\powered_cache_flush();
    }
    /**
    ...
    public function excludeAssetsHook($excludeAssets) {
        foreach (['js', 'css'] as $type) {
            add_filter(
                \sprintf('powered_cache_fo_%s_do_concat', $type),
                function ($doConcat, $handle) use ($excludeAssets, $type) {
