...
',
    ...
    protected function getTranslationSuffixByBasename($basename) {
        ...
        $languageFolder =
            $path . \DevOwl\RealCookieBanner\Vendor\MatthiasWeb\Utils\Constants::LOCALIZATION_PUBLIC_JSON_I18N;
        ...
        if (\file_exists($composerPath)) {
            ...
            if ($type === 'script') {
                ...
                $this->setLazyScriptTranslations(
                    $useHandle,
                    $rootSlug . '-' . $handle,
                    $pluginPath . $packageDir . 'languages/frontend/json'
                );
            } else {
                wp_enqueue_style($useHandle, $url, $deps, $cachebuster, $media);
            }
...
JS;
     ...
     */
    public function anonymous_localize_script(
        $handle,
        $object_name,
        $l10n,
        $makeBase64Encoded = [],
        ...
        add_filter(
            ...
            function ($tag, $scriptHandle) use (
                ...
                if ($scriptHandle === $handle) {
                    ...
                    $tag =
                        \sprintf(
                            '<script type="application/json" %4$s id="a%1$s1-js-extra">%2$s</script>
<script %4$s id="a%1$s2-js-extra">var %3$s = /* document.write */ JSON.parse(document.getElementById("a%1$s1-js-extra").innerHTML, %5$s);</script>',
                            $uuid,
                            wp_json_encode($l10n),
                            $object_name,
                            \join(' ', [
