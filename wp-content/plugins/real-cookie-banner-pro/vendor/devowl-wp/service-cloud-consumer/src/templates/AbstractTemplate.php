...
abstract class AbstractTemplate {
    ...
    public function fromArray($arr) {
        if (\is_array($arr)) {
            ...
            $this->recommendedWhenOneOf = \is_array($arr['recommendedWhenOneOf'] ?? null)
                ? $arr['recommendedWhenOneOf']
                : [];
            $this->extendsIdentifier = \is_string($arr['extendsIdentifier'] ?? null) ? $arr['extendsIdentifier'] : null;
            $this->status = \in_array($arr['status'] ?? null, [self::STATUS_DRAFT, self::STATUS_PUBLISHED], \true)
                ? $arr['status']
                : self::STATUS_PUBLISHED;
            $this->createdAt = \is_string($arr['createdAt'] ?? null) ? \strtotime($arr['createdAt']) : \time();
            $this->tier = \in_array($arr['tier'] ?? null, [self::TIER_FREE, self::TIER_PRO], \true)
                ? $arr['tier']
                : self::TIER_FREE;
    ...
    public static function toArray($template) {
        return \json_decode(\json_encode($template), ARRAY_A);
    }
    /**
