 ...
 */
class ServiceAvailableBlockerTemplatesMiddleware extends
    \DevOwl\RealCookieBanner\Vendor\DevOwl\ServiceCloudConsumer\middlewares\AbstractPoolMiddleware {
        ...
        if (
            ...
            isset(
                $typeClassToAllTemplates[
                    \DevOwl\RealCookieBanner\Vendor\DevOwl\ServiceCloudConsumer\templates\BlockerTemplate::class
                ]
            )
        ) {
            /**
            ...
            $tooltipText = $variableResolver->resolveDefault(
                'i18n.ServiceAvailableBlockerTemplatesMiddleware.tooltip',
            ...
            if (
                !\DevOwl\RealCookieBanner\Vendor\DevOwl\ServiceCloudConsumer\Utils::in_array_column(
                    $existingServices,
                    'identifier',
                    $template->identifier
                )
                ...
                foreach ($blockerTemplates as $blockerTemplate) {
                    ...
                    if (
                        $isDisabled ||
                        $blockerTemplate->isHidden ||
                        !\in_array($template->identifier, $blockerTemplate->serviceTemplateIdentifiers, \true) ||
                        \in_array($blockerTemplate->identifier, $existingBlockerIdentifiers, \true) ||
                        ($blockerTemplate->tier !==
                    ...
                    $contentBlockerTemplates[] = [
                        'identifier' => $blockerTemplate->identifier,
                        'name' => $blockerTemplate->name,
                        'subHeadline' => $blockerTemplate->subHeadline
                    ];
                }
            }
