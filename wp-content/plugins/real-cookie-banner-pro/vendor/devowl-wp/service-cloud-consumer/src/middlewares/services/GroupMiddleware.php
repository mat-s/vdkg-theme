...
class GroupMiddleware extends
    ...
    public function beforeUsingTemplate($template) {
        if (
            ...
            \is_string($template->group)
        ) {
            $template->group = $this->getVariableResolver()->resolveDefault(
                \sprintf('i18n.GroupMiddleware.group.%s', $template->group),
                \ucfirst($template->group)
            );
        }
    }
}
