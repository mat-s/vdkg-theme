...
class OneOfMiddleware extends
    ...
    public function beforePersistTemplate($template, &$allTemplates) {
        ...
        if (\is_array($template->enabledWhenOneOf) && \count($template->enabledWhenOneOf) > 0) {
            ...
            if ($isDisabled) {
                $disabledTooltip = $variableResolver->resolveDefault(
                    'i18n.OneOfMiddleware.disabledTooltip',
                    'This template is currently disabled because a dependency component is not installed or the desired function is not active.'
                );
        ...
        if (\is_array($template->recommendedWhenOneOf) && \count($template->recommendedWhenOneOf) > 0) {
            $template->consumerData['isRecommended'] = $variableResolver->resolveOneOf(
                $template->recommendedWhenOneOf,
                $template
            );
        ...
        if (
            ...
            \is_array($template->shouldUncheckContentBlockerCheckboxWhenOneOf) &&
            \count($template->shouldUncheckContentBlockerCheckboxWhenOneOf) > 0
        ) {
            $template->shouldUncheckContentBlockerCheckbox = $variableResolver->resolveOneOf(
                $template->shouldUncheckContentBlockerCheckboxWhenOneOf,
                $template
            );
        }
    }
