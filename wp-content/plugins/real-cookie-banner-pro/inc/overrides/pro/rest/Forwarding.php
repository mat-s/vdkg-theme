...
class Forwarding {
    ...
    public function rest_api_init() {
        ...
        register_rest_route($namespace, '/' . self::ENDPOINT_CONSENT_FORWARD, [
            'methods' => 'POST',
        ...
        register_rest_route($namespace, '/forward/endpoints', [
            'methods' => 'GET',
            'callback' => [$this, 'routeGetEndpoints'],
            'permission_callback' => [$this, 'permission_callback'],
            'args' => [
                'filter' => [
                    'type' => 'string',
                    'enum' => \DevOwl\RealCookieBanner\lite\Forwarding::ALL_ENDPOINT_FILTERS,
                    'default' => \DevOwl\RealCookieBanner\lite\Forwarding::ENDPOINT_FILTER_ALL
                ]
            ]
        ]);
        register_rest_route($namespace, '/forward/cookie/(?P<slug>[^/]+)', [
            'methods' => 'GET',
            'callback' => [$this, 'routeGetUniqueCookie'],
            'permission_callback' => [$this, 'permission_callback']
        ]);
    }
    /**
    ...
    public function routePost($request) {
        ...
        if (\DevOwl\RealCookieBanner\IpHandler::getInstance()->isFlooding() || !\is_array($cookies)) {
        ...
        $persist = \DevOwl\RealCookieBanner\lite\Forwarding::getInstance()->persist(
            $uuid,
            $consentId,
            $cookies,
            $buttonClicked,
    ...
    public function routeGetEndpoints($request) {
        return new \WP_REST_Response(
            \DevOwl\RealCookieBanner\lite\Forwarding::getInstance()->getAvailableEndpoints(
    ...
    public function routeGetUniqueCookie($request) {
        ...
        $result = \DevOwl\RealCookieBanner\lite\Forwarding::getInstance()->getByUniqueName($slug);
