...
class Presets {
    ...
    public function rest_api_init() {
        ...
        register_rest_route($namespace, '/templates/(?P<type>blocker|services)', [
            'methods' => 'GET',
            'callback' => [$this, 'routeTemplatesGet'],
            'permission_callback' => [$this, 'permission_callback'],
            'args' => [
                'storage' => ['type' => 'enum', 'enum' => ['use', 'invalidate', 'redownload'], 'default' => 'use']
            ]
        ]);
        register_rest_route($namespace, '/templates/(?P<type>blocker|services)/(?P<identifier>[a-zA-Z0-9_-]+)/', [
            'methods' => 'GET',
            'callback' => [$this, 'routeTemplatesGetSingle'],
            'permission_callback' => [$this, 'permission_callback']
        ]);
    ...
    public function routeTemplatesGet($request) {
        ...
        if ($storage === 'redownload') {
            \DevOwl\RealCookieBanner\templates\TemplateConsumers::getInstance()->currentForceRedownload();
        }
        $consumer =
            $type === 'blocker'
                ? \DevOwl\RealCookieBanner\templates\TemplateConsumers::getCurrentBlockerConsumer()
                : \DevOwl\RealCookieBanner\templates\TemplateConsumers::getCurrentServiceConsumer();
        return new \WP_REST_Response([
            'items' => \DevOwl\RealCookieBanner\Vendor\DevOwl\ServiceCloudConsumer\templates\AbstractTemplate::toArrays(
                $consumer->retrieve($storage === 'invalidate')
            )
        ]);
    }
    /**
