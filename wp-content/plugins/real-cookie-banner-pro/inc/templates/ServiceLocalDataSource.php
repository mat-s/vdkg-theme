 ...
 */
class ServiceLocalDataSource extends
    \DevOwl\RealCookieBanner\Vendor\DevOwl\ServiceCloudConsumer\datasources\LocalDataSource {
    /**
     * C'tor.
     *
    ...
    public function createDefaults() {
        $s = new \DevOwl\RealCookieBanner\Vendor\DevOwl\ServiceCloudConsumer\templates\ServiceTemplate(
            $this->getConsumer()
        );
        $cookieDuration = \DevOwl\RealCookieBanner\settings\Consent::getInstance()->getCookieDuration();
        $s->version = 2;
        ...
        $s->legalBasis = \DevOwl\RealCookieBanner\settings\Cookie::LEGAL_BASIS_LEGAL_REQUIREMENT;
        $s->isProviderCurrentWebsite = \true;
        $s->technicalDefinitions = [
            [
                'type' => 'http',
                'name' => \DevOwl\RealCookieBanner\MyConsent::COOKIE_NAME_USER_PREFIX . '*',
                'host' => 'main+subdomains',
