    ...
    public function persistCustomBypass($contextString, $customBypass) {
        ...
        $wpdb->query(
            ...
            $wpdb->prepare(
                ...
                $dayIso,
                $contextString,
                $customBypass === null ? 'none' : $customBypass
            )
        );
    }
    /**
    ...
    public function new_version_installation_after_3_4_13($installed) {
        ...
        if (
            \DevOwl\RealCookieBanner\Core::versionCompareOlderThan(
                ...
                '3.4.13',
                ['3.4.14', '3.5.0'],
                function () use ($wpdb, $table_name_stats_buttons_clicked) {
                    // phpcs:disable WordPress.DB.PreparedSQL
                    $exists =
                        $wpdb->get_var("SHOW TABLES LIKE '{$table_name_stats_buttons_clicked}'") ===
                        $table_name_stats_buttons_clicked;
                    // phpcs:enable WordPress.DB.PreparedSQL
                    if ($exists) {
                        // phpcs:disable WordPress.DB.PreparedSQL
                        $countRows = \intval(
                            $wpdb->get_var("SELECT COUNT(*) AS cnt FROM {$table_name_stats_buttons_clicked}")
                        );
                        // phpcs:enable WordPress.DB.PreparedSQL
