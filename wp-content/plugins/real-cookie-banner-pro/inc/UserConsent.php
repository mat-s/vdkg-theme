...
class UserConsent {
    ...
    public function purge() {
        ...
        $table_name_stats_buttons_clicked = $this->getTableName(
            \DevOwl\RealCookieBanner\Stats::TABLE_NAME_BUTTONS_CLICKED
        );
        ...
        return [
            ...
            'stats_terms' => $stats_terms,
            'stats_buttons_clicked' => $stats_buttons_clicked,
            'stats_custom_bypass' => $stats_custom_bypass
        ];
    }
    /**
    ...
    public function byCriteria($args, $returnType = self::BY_CRITERIA_RESULT_TYPE_JSON_DECODE) {
        ...
        if ($returnType === self::BY_CRITERIA_RESULT_TYPE_COUNT) {
            ...
            // phpcs:disable WordPress.DB
            $sql = \sprintf(
                'SELECT %s FROM (SELECT cs.id FROM %s AS cs WHERE %s ORDER BY cs.created DESC LIMIT %d, %d) AS cids %s ORDER BY c.created DESC',
                \join(',', $fields),
                $table_name,
                \join(' AND ', $where),
                $limitOffset,
    ...
    public function getPersistedContexts() {
        ...
        foreach ($contexts as $context) {
            $result[
                $context
            ] = \DevOwl\RealCookieBanner\settings\Revision::getInstance()->translateContextVariablesString($context);
        }
        return $result;
    }
    /**
