...
class Cookie {
    ...
    const SYNC_META_COPY = [
        \DevOwl\RealCookieBanner\settings\Cookie::META_NAME_UNIQUE_NAME,
    ...
    const SYNC_META_COPY_ONCE = [
        ...
        \DevOwl\RealCookieBanner\settings\Cookie::META_NAME_PROVIDER_LEGAL_NOTICE_URL
    ];
    ...
    public function register() {
        ...
        register_meta('post', self::META_NAME_UNIQUE_NAME, [
            'object_subtype' => self::CPT_NAME,
            'type' => 'string',
            'single' => \true,
            'show_in_rest' => \true
        ...
        register_meta('post', self::META_NAME_PROVIDER_LEGAL_NOTICE_URL, [
            'object_subtype' => self::CPT_NAME,
            'type' => 'string',
            'single' => \true,
            'show_in_rest' => \true
        ...
        if (\DevOwl\RealCookieBanner\Core::versionCompareOlderThan($installed, '3.4.13', ['3.4.14', '3.5.0'])) {
            ...
            if (\count($affectedPostIds) > 0) {
                ...
                foreach ($result as $row) {
                    if (
                        !empty($row['consentForwardingUniqueNameMetaId']) &&
                        !empty($row['consentForwardingUniqueName'])
                    ) {
                        // Rename meta_key to `uniqueName`´
                        $wpdb->query(
                            // phpcs:disable WordPress.DB.PreparedSQL
                            $wpdb->prepare(
                                "UPDATE {$wpdb->postmeta} SET meta_key = 'uniqueName', meta_value = %s WHERE meta_id = %d",
                                $row['potentialUniqueName'],
                                $row['consentForwardingUniqueNameMetaId']
                            ),
                            ARRAY_A
                        );
                        ...
                        $wpdb->query(
                            // phpcs:disable WordPress.DB.PreparedSQL
                            $wpdb->prepare(
                                "INSERT IGNORE INTO {$wpdb->postmeta} (`post_id`, `meta_key`, `meta_value`) VALUES (%d, 'uniqueName', %s)",
                                $row['ID'],
                                $row['potentialUniqueName']
                            ),
                            ARRAY_A
    ...
    /**
     * Modify already given consents and adjust the metadata field for legal notice URL for "List of consents".
     *
    ...
    public static function applyMetaLegalNoticeUrlBackwardsCompatibility($revision, $independent) {
        if (!$independent && isset($revision['groups'])) {
            foreach ($revision['groups'] as &$group) {
                if (isset($group['items'])) {
                    foreach ($group['items'] as &$cookie) {
                        if (!isset($cookie[self::META_NAME_PROVIDER_LEGAL_NOTICE_URL])) {
                            $cookie[self::META_NAME_PROVIDER_LEGAL_NOTICE_URL] = '';
                        }
                    }
                }
            }
